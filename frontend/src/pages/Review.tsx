...
import { ArrowLeft, ArrowRight, Check, ChevronDown, Eye, Leaf, Pause, RotateCcw, SkipForward, Sprout, X } from 'lucide-react'
...
import { ContentField } from '../components/Editor'
...
function ReviewCard({ item, active, onWorking, onRated, onSkip, onConflict }: {
  ...
  return <section ref={cardRef} className="review-card panel"><div className="review-card-meta"><div><span className={`subject-badge ${subjects.find(s => s.id === item.subject)?.color}`}>{subjectName(item.subject)}</span>{item.is_mistake && <span className="mistake-badge">错题重做</span>}{item.schedule.state === 'new' && <span className="new-badge">初次学习</span>}</div><button className="text-button" disabled={busy || retry || uploading} onClick={onSkip}><SkipForward size={15}/>暂时跳过</button></div><div className="review-question"><span className="eyebrow">TAKE A MOMENT TO RECALL</span><h1>{item.title}</h1><Content text={item.question} media={item.question_media}/></div>
